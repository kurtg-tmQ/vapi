...
def replace_element_text(element: _Element, include_formatting: bool) -> str:
    ...
    if include_formatting and element.text:
        if element.tag == "head":
            ...
            # Create the header text with Markdown formatting
            header_text = f'{"#" * number} {elem_text}'
            
            # Determine the length of the header text
            header_length = len(header_text)
            
            # Create a line of dashes with the same length
            dash_line = '-' * header_length
            
            # Combine the header text, dash line, and a newline
            elem_text = f'\n{header_text}\n{dash_line}\n'
